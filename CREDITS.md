...
- **@Jodi Paige-Lee — "a must in a markdown file is a request; a must in code is a constraint."**
  This is why `verify.mjs` exists at all. Every "must" in `rules.md` is a gate, not a sentence.
- **@Gabriel Azoulay — a self-tested checker with negative fixtures.** `verify.mjs --selftest` runs a
  good map plus one bad fixture per gate, each failing on its named check, so the gates are proven to
  bite from a cold clone.
...
- **@Marcelo — the load-vs-verify split.** The folder the model reads (`identity`/`rules`/`examples`/
  `reference`) is kept separate from `fixtures/` (the checker's own material). The cartographer
  never sees its test territory.
- **@Greg Faysash — script computes, model labels** (carried over from my Regression Historian, Comp
  #10). The model writes the cards; `verify.mjs` mechanically resolves every citation to a real
  `file:line` and rejects photocopies. The model never gets to *assert* a citation is real.
